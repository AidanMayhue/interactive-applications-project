...
if page=="Calendar":
    ...
    st.divider()
    st.subheader("Remove Events")
    if st.session_state.events:
        with st.form("delete_form"):
            event_titles = [e['title'] for e in st.session_state.events]
            event_to_delete = st.multiselect("Select event to remove:", event_titles)
            delete_submit = st.form_submit_button("Delete Selected Event", type="primary")
            if delete_submit:
                if event_to_delete:
                    st.session_state.events = [e for e in st.session_state.events if e['title'] not in event_to_delete]
